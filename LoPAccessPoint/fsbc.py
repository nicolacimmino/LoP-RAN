# FSBC provides a wrapper for FSBC protocol implementation.
...
class fsbc:
  ...
  btclient = btsync.Client( host='127.0.0.1', port='1106', username='admin', password='password')
  ...
  def __init__(self, uniqueid, received_data_callback):
      ...
      btclient.add_sync_folder(self.ourpath, self.uniqueid)
  ...
  def monitorFolder(self):
    while(True):
      ...
      if file.endswith(".fsbc_oi"):
        ...
        # Notify the new data
        received_data_callback(self.uniqueid, senderid, message, replyreuested, connectiontype)
   ...
   def sendMessage(self, peerid, message, replyreuested="noreply", connectiontype="permanent"):
      ...
      if not os.path.exists(peerpath):
        ...
        # Acquire our own folder
        btclient.add_sync_folder(peerpath, peerid)
